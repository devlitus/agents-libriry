...
- [ ] Throws clear error if Ollama is unavailable
...
- [ ] Tests for all 3 providers pass with mocks
...
- `IndexerConfig`:
  ...
  - `alwaysRead: string[]` — mandatory context files
- `DetectedProject`:
  - `language: string`
...
- Read `alwaysRead` files from config
...
- `AgentResult`: generic type for agent result
...
|------|:----------:|:--------:|
| 1.1 LLM Abstraction | L | 7 |
| 1.2 SQLite Memory | L | 7 |
| 1.3 Indexer | L | 9 |
